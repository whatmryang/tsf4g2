...
/**计算shtable的大小
 *@param[in]	 a_iBuck  桶数
 *@param[in]  a_iMax   元素个数
 *@param[in]  a_iUnit  每个元素的大小
 *@retval  0  失败
 ...
 */
LPSHTABLE
sht_init(void* pvBuff, size_t iBuff, int iBucket, int iMax, int iUnit);
 ...
 */
void* sht_find(LPSHTABLE pstTab, const void* pvData, SHT_CMP pfnCmp,
		SHT_HASHCODE pfnHashCode);
 ...
 *@note   pvData只需要填入与pfnCmp和pfnHashCode 相关的部分即可
 */
void* sht_remove(LPSHTABLE pstTab, const void* pvData, SHT_CMP pfnCmp,
		SHT_HASHCODE pfnHashCode);
