...
#define RESERVE_SIZE		256
	...
	if (0 == iRet)
	{
        ...
        DispContent* target = (DispContent*)pszHost;
		if (0 != iRet)
		{
	...
	if (0 == iRet)
	{
		...
		for (i = 0 ; i < stOption.iLoop; i++)
		{
            ...
            DispContent* DISP = (DispContent*)pszHost;
            /*
            printf("%d\n", sizeof(DISPCONTENT));
            printf("%d\n", sizeof(DispContent));
            ...
            */
            TdrWriteBuf stEncodeBuf(pszNet, iSize);
            TdrError::ErrorType ret = DISP->pack(stEncodeBuf);
            if (TdrError::TDR_NO_ERROR != ret)
            {
                printf("failed to tdr pack, for %s", TdrError::getErrorString(ret));
                return -1;
            }
	...
	if (0 == iRet)
	{
		...
		for (i = 0 ; i < stOption.iLoop; i++)
		{
            ...
            */
            memset(pszHost, 0, iSize);
            TdrReadBuf stDecodeBuf(pszNet, iSize);
            DispContent* target = (DispContent*)pszHost;
            TdrError::ErrorType ret = ((DispContent*)pszHost)->unpack(stDecodeBuf);
            if (TdrError::TDR_NO_ERROR != ret)
            {
                printf("failed to tdr unpack, for %s", TdrError::getErrorString(ret));
                return -1;
            }
		}/*for (i = 0 ; i < stOption.iXMLFileNum; i++)*/
