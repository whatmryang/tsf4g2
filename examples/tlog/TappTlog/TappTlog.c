...
static int TappTlog_init(TAPPCTX *pstAppCtx, TAPPTLOGENV *pstEnv)
{
	...
	pstEnv->pstRunCumu = (TAPPTLOGRUN_CUMULATE *)pstAppCtx->stRunDataCumu.pszBuff;
	if (NULL == pstEnv->pstRunCumu)
	{
		pstEnv->pstRunCumu = &gs_stCumu;
	}

	pstEnv->pstRunStat = (TAPPTLOGRUN_STATUS *)pstAppCtx->stRunDataStatus.pszBuff;
...
static int TappTlog_proc(TAPPCTX *pstAppCtx, TAPPTLOGENV *pstEnv)
{
	int iBusy=0;
...
static int TappTlog_reload(TAPPCTX *pstAppCtx, TAPPTLOGENV *pstEnv)
{
...
static int TappTlog_fini(TAPPCTX *pstAppCtx, TAPPTLOGENV *pstEnv)
{
...
static int TappTlog_tick(TAPPCTX *pstAppCtx, TAPPTLOGENV *pstEnv)
{
	return 0;
}

static int TappTlog_stop(TAPPCTX *pstAppCtx, TAPPTLOGENV *pstEnv)
{
	int iReady=0;
