...
//version info
#define  TDR_VER_MAJOR             1
#define  TDR_VER_MINOR             2
#define  TDR_VER_REV               0
...
int tdr_parse_option(OUT LPTDRDR2XMLOPTION a_pstOption, IN int a_argc, IN char *a_argv[])
{
	...
	while (1)
	{
		...
		case 'v':
			{
				iRet = 1;
                printf("%s:version %d.%d.%d  build at %s.\n", basename(a_argv[0]), TDR_VER_MAJOR,
                    TDR_VER_MINOR, TDR_VER_REV, __DATE__);	
                exit(0);
				break;
			}
		default:
