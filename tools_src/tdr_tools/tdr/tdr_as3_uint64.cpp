...
TTransAS3::makeUint64()
{
    ...
    lines.push_back("package " + this->packageName + " {");
    {
        ...
        lines.push_back("");
        lines.push_back(indent + "public class UInt64 {");
        {
            makeUint64_i(2, lines);
        }
        lines.push_back(indent + "}");
    }
    lines.push_back("}");
    this->output(lines, string("UInt64.as"));
}

void
TTransAS3::makeUint64_i(unsigned int indentLevel, list<string>& lines)
{
    const string& indent = getIndentStr(indentLevel);
    lines.push_back(indent + "private var _high : uint = 0;");
    ...
    lines.push_back(indent + "public function UInt64(high:uint = 0, low:uint = 0) {");
    ...
    lines.push_back(indent + "public function decode(buf:ByteArray):int {");
    {
        ...
        lines.push_back(indent + "if (buf.bytesAvailable < UInt64.size) {");
        {
            const string& indent = getIndentStr(indentLevel + 2);
            lines.push_back(indent + "return ErrorMap.ERR_PARTIAL_MSG;");
        }
        lines.push_back(indent + "}");
        lines.push_back(indent + "var high:int = buf.readUnsignedInt();");
    ...
    {
        const string& indent = getIndentStr(indentLevel + 1);
        lines.push_back(indent + "buf.writeUnsignedInt(_high);");
    ...
    {
        const string& indent = getIndentStr(indentLevel + 1);
        lines.push_back(indent + "return UInt64.size;");
    }
    lines.push_back(indent + "}");

    lines.push_back("");

    lines.push_back(indent + "public function get high():uint {");
    {
        const string& indent = getIndentStr(indentLevel + 1);
    ...
    lines.push_back(indent + "public function setValue(high:uint, low:uint):void {");
    {
        const string& indent = getIndentStr(indentLevel + 1);
    ...
    lines.push_back(indent + "public function compare(nAnother:UInt64):int {");
    {
        const string& indent = getIndentStr(indentLevel + 1);
