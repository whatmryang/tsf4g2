...
sed -i  -e "s/^\(#define\\s*MAJOR\\s*\)[0-9]*/\1$Major/g" version.h

#replace minor macro definition in version.h
sed -i  -e "s/^\(#define\\s*MINOR\\s*\)[0-9]*/\1$Minor/g" version.h 

#replace rev macro definition in version.h
sed -i  -e "s/^\(#define\\s*REV\\s*\)[0-9]*/\1$Rev/g" version.h 

#replace build macro definiton in version.h
sed -i  -e "s/^\(#define\\s*BUILD\\s*\)[0-9]*/\1$Build/g" version.h 

cp version.h ${TSF4G_DIR}/services_src/tlog_service/
cp version.h ${TSF4G_DIR}/services_src/tbus_service/

echo "include ../tsf4g.mk" >version.mk
echo "TSF4G_LIB_MAJOR=$Major">>version.mk
echo "TSF4G_LIB_MINOR=$Minor">>version.mk
echo "TSF4G_LIB_PATCH=$Rev">>version.mk
cp version.mk ${TSF4G_DIR}/lib_src/Makefile.pub
echo "done"
