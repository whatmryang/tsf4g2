...
sed -i  -e "s/^\(#define\\s*TORM_MAJOR\\s*\)[0-9]*/\1$Major/g" ../apps/torm/tormapi/Makefile

#replace major macro defintion in version.h
sed -i  -e "s/^\(#define\\s*MAJOR\\s*\)[0-9]*/\1$Major/g" ../apps/torm/tormsvr/tormsvr_version.h

#replace minor macro definition in version.h
sed -i  -e "s/^\(#define\\s*MINOR\\s*\)[0-9]*/\1$Minor/g" ../apps/torm/tormsvr/tormsvr_version.h 

#replace rev macro definition in version.h
sed -i  -e "s/^\(#define\\s*REV\\s*\)[0-9]*/\1$Rev/g" ../apps/torm/tormsvr/tormsvr_version.h

#replace build macro definiton in version.h
sed -i  -e "s/^\(#define\\s*BUILD\\s*\)[0-9]*/\1$Build/g" ../apps/torm/tormsvr/tormsvr_version.h
