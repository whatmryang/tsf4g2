...
sed -i  -e "s/^\(#define\\s*MAJOR\\s*\)[0-9]*/\1$Major/g" ../apps/tdir/tdirsvr/tdir_version.h

#replace minor macro definition in version.h
sed -i  -e "s/^\(#define\\s*MINOR\\s*\)[0-9]*/\1$Minor/g" ../apps/tdir/tdirsvr/tdir_version.h 

#replace rev macro definition in version.h
sed -i  -e "s/^\(#define\\s*REV\\s*\)[0-9]*/\1$Rev/g" ../tsf4g/apps/tdir/tdirsvr/tdir_version.h 

#replace build macro definiton in version.h
sed -i  -e "s/^\(#define\\s*BUILD\\s*\)[0-9]*/\1$Build/g" ../tsf4g/apps/tdir/tdirsvr/tdir_version.h 
